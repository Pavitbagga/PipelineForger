...
backend/
  src/
    ...
    lib/
      claude.ts             Single askClaude(system, user) wrapper (change model here)
...
- **All Claude calls** go through `backend/src/lib/claude.ts:askClaude()` — update the model or add retries in one place.
